...
## detection covariate) JAGS model on simulated data.
## Time-stamp: <2018-12-04 12:55:24 (slane)>
################################################################################
################################################################################
library(here)
library(dplyr)
...
jags.data <- list(
    n = nrow(la1_ld1_data), count = la1_ld1_data[["D"]],
    x_arrival = la1_ld1_data[["xvar_arrival"]],
    x_detection = la1_ld1_data[["xvar_detection"]]
)
la1_ld1_fit <- jags(
    data = jags.data,
...
la1_ld1_samples <- as.matrix(la1_ld1_fit$samples, chains = TRUE) %>%
    as_tibble() %>%
    rename(Chain = CHAIN)
## plot a selection of posterior estimates
inds <- sample(1:100, 9)
pars <- paste0("N[", inds, "]")
Nsamps <- la1_ld1_samples %>%
    select(pars) %>%
    gather(site, N)
Nact <- tibble(
    site = pars,
    N = la1_ld1_data[["N"]][inds]
)
...
pdf(file = here("figs/jags-testing/la1-ld1-tests.pdf"))
pl_N
mcmc_hist(la1_ld1_samples, pars = "alpha_arrival") +
    geom_vline(xintercept = 10, col = "maroon")
mcmc_hist(la1_ld1_samples, pars = "beta_arrival") +
    geom_vline(xintercept = 10, col = "maroon")
mcmc_hist(la1_ld1_samples, pars = "alpha_detection") +
    geom_vline(xintercept = -1, col = "maroon")
mcmc_hist(la1_ld1_samples, pars = "beta_detection") +
    geom_vline(xintercept = 1, col = "maroon")
dev.off()
## Samples
summaries <- la1_ld1_samples %>%
