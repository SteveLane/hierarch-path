...
## arrival covariate) plus site-specific intercept JAGS model on simulated
## data. Cannot get this one running at the moment.
## Time-stamp: <2018-12-04 12:52:39 (slane)>
################################################################################
################################################################################
library(here)
library(dplyr)
...
la1_ld4_data <- sim_detections(
    arrival_model = linear_arrival1,
    detection_model = linear_detection4,
    T = 10,
    S = 10,
    r = 1,
    D = 75,
...
jags.data <- list(
    n = nrow(la1_ld4_data), count = la1_ld4_data[["D"]],
    x_arrival = la1_ld4_data[["xvar_arrival"]],
    x_detection = la1_ld4_data[["xvar_arrival"]],
    site = la1_ld4_data[["site"]], nsite = max(la1_ld4_data[["site"]])
)
la1_ld4_fit <- jags(
    data = jags.data,
...
la1_ld4_samples <- as.matrix(la1_ld4_fit$samples, chains = TRUE) %>%
    as_tibble() %>%
    rename(Chain = CHAIN)
## plot a selection of posterior estimates
inds <- sample(1:100, 9)
pars <- paste0("N[", inds, "]")
Nsamps <- la1_ld4_samples %>%
    select(pars) %>%
    gather(site, N)
Nact <- tibble(
    site = pars,
    N = la1_ld4_data[["N"]][inds]
)
...
pdf(file = here("figs/jags-testing/la1-ld4-tests.pdf"))
pl_N
mcmc_hist(la1_ld4_samples, pars = "sigma.eta") +
    geom_vline(xintercept = 0.5, col = "maroon")
mcmc_hist(la1_ld4_samples, pars = "alpha_arrival") +
    geom_vline(xintercept = 10, col = "maroon")
mcmc_hist(la1_ld4_samples, pars = "beta_arrival") +
    geom_vline(xintercept = 10, col = "maroon")
mcmc_hist(la1_ld4_samples, pars = "alpha_detection") +
    geom_vline(xintercept = -3, col = "maroon")
mcmc_hist(la1_ld4_samples, pars = "beta_detection") +
    geom_vline(xintercept = 0.6, col = "maroon")
dev.off()
## Samples
summaries <- la1_ld4_samples %>%
